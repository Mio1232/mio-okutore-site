...
exports.handler = async (event) => {
  ...
  if (images.length) {
    ...
    const targetLine =
      focus === 'gold'
        ? '【対象】この記事はゴールド（XAUUSD）のみを扱います。貼られたチャートはすべてゴールドとして読み取り、ドル円には触れないでください。'
      : focus === 'both'
        ? '【対象】この記事はドル円（USD/JPY）とゴールド（XAUUSD）の両方を扱います。各チャートの銘柄を画像から判断し、銘柄ごとに<h3>の見出しを分けて分析してください。'
        : '【対象】この記事はドル円（USD/JPY）のみを扱います。貼られたチャートはすべてドル円として読み取り、ゴールドには触れないでください。';
    const multiLine =
      focus === 'both'
        ? '複数枚ある場合は、同じ銘柄の異なる時間足はMTF（大きな流れ＝上位足と直近の目線＝下位足）で関連づけ、異なる銘柄は銘柄ごとに分けて分析してください。'
        : '複数枚ある場合は、すべて同じ銘柄の異なる時間足として、大きな流れ（上位足）と直近の目線（下位足）を関連づけてMTFで分析してください。';
    const imagePrompt =
