...
exports.handler = async (event) => {
  ...
  if (images.length) {
    // ===== チャート画像モード：画像を読み取り、写っている価格をもとに記事化（Web検索なし）=====
    useWebSearch = false;
    maxTokens = 4000;
    ...
    const imagePrompt =
      ...
      (ctx ? ('【補足メモ】' + ctx + '\n') : '') +
      '複数枚ある場合は、それぞれの時間足を踏まえ、大きな流れ（上位足）と直近の目線（下位足）を関連づけて書いてください。\n\n' +
      ...
      '  "table_html": "チャートから読み取った主要レベルとシナリオをまとめた<table>のHTML（属性やclassは付けない）。まず読み取った現在値付近・主要なサポート/レジスタンスの価格を行にし、続けて上昇シナリオ・下落シナリオの想定を入れる。"\n' +
      '}\n\n' +
      'ルール：\n' +
